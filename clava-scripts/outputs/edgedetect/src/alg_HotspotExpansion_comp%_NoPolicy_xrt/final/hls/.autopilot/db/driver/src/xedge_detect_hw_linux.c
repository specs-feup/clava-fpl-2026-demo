...
typedef struct {
    ...
    XEdge_detect_hw_uio_map maps[ MAX_UIO_MAPS ];
} XEdge_detect_hw_uio_info;

/***************** Variable Definitions **************************************/
static XEdge_detect_hw_uio_info uio_info;
...
static int uio_info_read_name(XEdge_detect_hw_uio_info* info) {
    char file[ MAX_UIO_PATH_SIZE ];
    sprintf(file, "/sys/class/uio/uio%d/name", info->uio_num);
    return line_from_file(file, info->name);
}

static int uio_info_read_version(XEdge_detect_hw_uio_info* info) {
    char file[ MAX_UIO_PATH_SIZE ];
    sprintf(file, "/sys/class/uio/uio%d/version", info->uio_num);
    return line_from_file(file, info->version);
}

static int uio_info_read_map_addr(XEdge_detect_hw_uio_info* info, int n) {
    int ret;
    char file[ MAX_UIO_PATH_SIZE ];
...
static int uio_info_read_map_size(XEdge_detect_hw_uio_info* info, int n) {
    int ret;
    char file[ MAX_UIO_PATH_SIZE ];
...
int XEdge_detect_hw_Initialize(XEdge_detect_hw *InstancePtr, const char* InstanceName) {
	XEdge_detect_hw_uio_info *InfoPtr = &uio_info;
