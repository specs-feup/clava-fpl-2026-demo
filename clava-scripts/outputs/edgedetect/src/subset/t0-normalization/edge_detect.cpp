...
void rgbToGrayscale(int input_image[512][1536], int output_image[512][512])
{
    for (int i = 0; i < 512; i++)
    {
        ...
        for (int j = 0; j < 512; j++)
        {
            ...
            r = input_image[i][jj];
            int g;
            g = input_image[i][jj + 1];
            int b;
            b = input_image[i][jj + 2];
            jj = jj + 3;
            float gray;
    ...
    for (r = 0; r < 512 - 3 + 1; r++)
    {
        for (c = 0; c < 512 - 3 + 1; c++)
        {
            ...
            for (i = 0; i < 3; i++)
            {
                for (j = 0; j < 3; j++)
                {
                    sum = sum + input_image[(r + i)][(c + j)] * filter[i][j];
                }
            }
...
void edge_detect(int image_rgb[512][1536], int image_gray[512][512], int temp_buf[512][512], int filter[3][3],
                 int output[512][512])
{
