...
typedef struct {
    ...
    XConvolve2d_rep0_hw_uio_map maps[ MAX_UIO_MAPS ];
} XConvolve2d_rep0_hw_uio_info;

/***************** Variable Definitions **************************************/
static XConvolve2d_rep0_hw_uio_info uio_info;
...
static int uio_info_read_name(XConvolve2d_rep0_hw_uio_info* info) {
    char file[ MAX_UIO_PATH_SIZE ];
    sprintf(file, "/sys/class/uio/uio%d/name", info->uio_num);
    return line_from_file(file, info->name);
}

static int uio_info_read_version(XConvolve2d_rep0_hw_uio_info* info) {
    char file[ MAX_UIO_PATH_SIZE ];
    sprintf(file, "/sys/class/uio/uio%d/version", info->uio_num);
    return line_from_file(file, info->version);
}

static int uio_info_read_map_addr(XConvolve2d_rep0_hw_uio_info* info, int n) {
    int ret;
    char file[ MAX_UIO_PATH_SIZE ];
...
static int uio_info_read_map_size(XConvolve2d_rep0_hw_uio_info* info, int n) {
    int ret;
    char file[ MAX_UIO_PATH_SIZE ];
...
int XConvolve2d_rep0_hw_Initialize(XConvolve2d_rep0_hw *InstancePtr, const char* InstanceName) {
	XConvolve2d_rep0_hw_uio_info *InfoPtr = &uio_info;
